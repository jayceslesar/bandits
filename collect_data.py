    ...
    for i, arms in enumerate((easy_arms, hard_arms)):
        for arm in arms.arms:
            ...
            try:
                ...
                regret_str = ""
            fig.add_trace(go.Scatter(x=x, y=y, name=f"Arm {arm} {regret_str}", legendgroup=i), row=i + 1, col=1)
            fig.update_xaxes(title_text="Turn", row=i + 1, col=1)
            fig.update_yaxes(title_text="Reward", row=i + 1, col=1)

    fig.update_layout(title_text=f"Rewards for {strategy_name.title()} Strategy", legend_tracegroupgap=300)
    fig.show()
